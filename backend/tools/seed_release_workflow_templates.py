"""Seed fuer Release-Workflow-Templates (Phase A).

Verhalten:
- idempotent
- default: dry-run
- keine Router-SQL, Tool fuer kontrollierte Initialisierung

Usage:
  python backend/tools/seed_release_workflow_templates.py
  python backend/tools/seed_release_workflow_templates.py --apply
  python backend/tools/seed_release_workflow_templates.py --apply --force-update
"""
...
async def _load_by_uid(conn: asyncpg.Connection, spec: TemplateSpec) -> Dict[str, Any] | None:
...
async def _load_by_name(conn: asyncpg.Connection, spec: TemplateSpec) -> Dict[str, Any] | None:
    row = await conn.fetchrow(
...
async def _insert_record(conn: asyncpg.Connection, spec: TemplateSpec, data: Dict[str, Any]) -> None:
...
async def _update_record(conn: asyncpg.Connection, spec: TemplateSpec, data: Dict[str, Any]) -> None:
    payload = json.dumps(data, ensure_ascii=False)
    await conn.execute(
        f"""
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"=== Seed Release Workflow Templates ({mode}) ===")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed release workflow templates")
